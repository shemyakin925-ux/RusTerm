...
def test_cli_full_cycle_init_ingest_snapshot_export_verify_doctor(capsys):
    ...
    try:
        ...
        assert "schema_version=43" in out
        ...
        assert "со значением 4, пусто 23" in out
        ...
        assert report["schema_version"] == 43
    finally:
        shutil.rmtree(root)
...
def test_cli_doctor_reports_schema_drift(capsys):
    ...
    числами — применённой версией и ожидаемой. Здесь: база эпохи TASK-12,
    остановившаяся на 36 (нет ни issuer_ingest_state, ни индексов)."""
    root = _root()
    try:
        main(["--root", root, "init"])
        capsys.readouterr()
        ...
        assert any("schema_version=36" in p and "43" in p
                   for p in report["problems"])
    finally:
        shutil.rmtree(root)


def test_cli_doctor_reports_schema_drift_38(capsys):
    """TASK-14 A1: миграция 38 (индексы) в doctor видна ровно как 37 —
    той же силой, без исключений: база без 38 отстаёт от кода."""
    root = _root()
    try:
        main(["--root", root, "init"])
        capsys.readouterr()
        ...
        assert any("schema_version=37" in p and "43" in p
                   for p in report["problems"])
    finally:
        shutil.rmtree(root)


def test_cli_doctor_reports_schema_drift_39(capsys):
    """TASK-17 E3: миграция 39 (industry_aggregate) видна doctor'у ровно
    как предыдущие — той же силой, без исключений."""
    root = _root()
    try:
        main(["--root", root, "init"])
        capsys.readouterr()
        ...
        assert any("schema_version=38" in p and "43" in p
                   for p in report["problems"])
    finally:
        shutil.rmtree(root)
...
def test_cli_metrics_budget_watchlist_import_export(capsys):
    ...
    try:
        ...
        # ingest --source edgar: у демо-эмитента нет CIK — честный отказ
        assert main(["--root", root, "ingest", "--instrument", "US-CLI-DEMO",
                     "--source", "edgar"]) == 1
        err = capsys.readouterr().err
        assert "нет CIK" in err
        assert "rusterm add" in err
    finally:
        shutil.rmtree(root)
...
def test_u9_status_after_demo_flow_reports_snapshot_and_coverage(capsys):
    ...
    try:
        ...
        assert payload["schema_version"] == 43
...
def test_cli_doctor_reports_schema_drift_40(capsys):
    ...
    try:
        ...
        assert any("schema_version=39" in p and "43" in p
                   for p in report["problems"])
    finally:
        shutil.rmtree(root)
