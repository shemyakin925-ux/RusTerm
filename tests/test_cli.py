...
def test_cli_full_cycle_init_ingest_snapshot_export_verify_doctor(capsys):
    ...
    try:
        ...
        assert "schema_version=38" in out
...
def test_cli_doctor_reports_schema_drift(capsys):
    ...
    try:
        ...
        report = json.loads(capsys.readouterr().out)
        assert report["ok"] is False
        # после удаления 37 и 38 максимум — 36 (миграция 37 —
        # issuer_ingest_state, TASK-13 Z2; 38 — индексы, TASK-14 A1)
        assert report["schema_version"] == 36
        assert any("schema_version=36" in p and "38" in p
...
def test_cli_doctor_reports_schema_drift_38(capsys):
    ...
    try:
        ...
        report = json.loads(capsys.readouterr().out)
        assert report["ok"] is False
        # после удаления свежей версии 38 максимум — 37
        assert report["schema_version"] == 37
        assert any("schema_version=37" in p and "38" in p
                   for p in report["problems"])
    finally:
        shutil.rmtree(root)
...
def test_u9_status_after_demo_flow_reports_snapshot_and_coverage(capsys):
    ...
    try:
        ...
        assert payload["schema_version"] == 38
