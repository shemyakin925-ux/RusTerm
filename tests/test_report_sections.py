...
def _l3_missing(done_ids: list[str], log_text: str,
    ...
    for block in log_text.split("\x1e"):
        ...
        #
        # Круг 108: над маркером N+1 легли коммиты координатора (его
        # ход — это и есть круг N+1). Прежнее условие «пропустить
        # маркер N+1, только если выше него ничего не было» принимало
        # его за нижнюю границу, и окно сжималось до координаторских
        # коммитов. Правило по существу: всё, что НОВЕЕ маркера круга
        # round_no + 1, — работа следующего круга, чем бы оно ни было;
        # собранное до него отбрасывается. Маркер любого ДРУГОГО круга
        # (не N и не N+1) — жёсткая остановка без окна: номера между
        # ними пропали, и доказать, что коммит принадлежит кругу N,
        # нечем (зуб test_leading_marker_of_another_round_is_not_a_
        # lower_bound).
        #
        # Маркер НИЖЕ round_no (круг 107 так и остался без маркера:
        # неудачный hand координатора оставил смену номера в индексе,
        # и она уехала внутри коммита «Координатор: …», f8534a4) —
        # ближайшая доступная нижняя граница: коммиты под маркером N+1
        # и над ним всё равно не старше круга N. Корневая починка —
        # отдельный пункт: relay обязан всегда оставлять маркер.
        relay = re.search(r"Эстафета: круг (\d+),", sha_subject)
        if round_no is not None and relay:
            number = int(relay.group(1))
            if number == round_no + 1:
                named = {}
                work_seen = False
                continue
...
# ТЗ-88 C2: литерал вместо живой истории. Прежние зубы звали
# `_git_log_name_only()` и держались лишь на том, что коммита с именем
# `Z9` на ветке нет: в дереве, которое не git-репозиторий, они падали с
# CalledProcessError, а один случайный коммит «Z9: …» с кодом делал их
# красными без всякого промаха стража. Здесь граница круга тоже своя, и
# Z9 лежит ПОД ней — то есть отсутствует именно по вине границы.
Z9_LOG = (
    "\x1e1111111 ТЗ-96 R5: GUIDE 1.1 — первые пятнадцать минут\n"
    "\n"
    "GUIDE.md\n"
    "tests/test_guide_truth.py\n"
    "\x1e2222222 Эстафета: круг 121, ход у executor\n"
    "\n"
    "agent/BATON.json\n"
    "\x1e0000000 ТЗ-76 Z9: тот же идентификатор в прошлом круге\n"
    "\n"
    "rusterm/old.py\n"
)
...
def test_l3_flags_an_item_that_no_commit_carries():
    """Z9 нет в окне круга, хотя ниже границы он лежит старым коммитом с
    кодом. Живая история ветки в решении не участвует: лог литеральный,
    номер круга назван явно, фолбэк индекса отключён пустым
    staged-списком."""
    assert _l3_missing(["Z9"], Z9_LOG, [], round_no=121) == ["Z9"]


def test_l2_staged_fallback_only_opens_for_a_non_test_file():
    """На том же литерале вердикт решает только переданный staged-список:
    тестовый файл фолбэк не открывает, нетестовый — открывает."""
    assert _l3_missing(["Z9"], Z9_LOG, ["tests/test_x.py"],
                       round_no=121) == ["Z9"]
    assert _l3_missing(["Z9"], Z9_LOG, ["rusterm/x.py"],
                       round_no=121) == []
...
# ── ТЗ-89 D3: зубы на новые правила границы ─────────────────────────────

# Работа круга 105 лежит МЕЖДУ маркерами 105 и 106; над маркером 106 —
# коммиты следующего круга (ход координатора и есть круг 106).
FAKE_LOG_COMMITS_ABOVE_THE_NEXT_MARKER = (
    "\x1e9999999 ТЗ-89 K1: реализация следующего круга, выше маркера\n"
    "\n"
    "rusterm/next_round.py\n"
    "\x1e8888888 Эстафета: круг 106, ход у coordinator — agent/TASK-89.md\n"
    "\n"
    "agent/BATON.json\n"
    "\x1e7777777 ТЗ-88 C8: настоящая работа круга 105\n"
    "\n"
    "rusterm/in_round.py\n"
    "\x1e3333333 Эстафета: круг 105, ход у executor — agent/TASK-88.md\n"
    "\n"
    "agent/BATON.json\n"
)


def test_a_commit_above_the_next_marker_is_not_the_rounds_work():
    """ЗУБ D3 (первый): всё, что НОВЕЕ маркера круга N+1, — работа
    следующего круга, чем бы оно ни было: и маркерным коммитом, и
    обычным. K1 назван коммитом над маркером 106 — кругу 105 он не
    принадлежит. Второй строкой закреплена обратная половина того же
    правила: настоящая работа круга (C8, под маркером 106) кредит
    получает — отбрасывается верх, а не окно.

    На прежнем коде (6cf14ce, до починки 46b1559) краснеет первая
    строка: маркер N+1, над которым что-то уже лежало, принимался за
    нижнюю границу, обзор обрывался с собранным `named` — и работа
    следующего круга закрывала пункт этого круга. Замер того же
    литерала прежним кодом: `OLD K1: []` там, где зуб требует
    `['K1']` (и `OLD C8: ['C8']` — та самая ложная краснота приёмки,
    из-за которой круг 108 и встал)."""
    assert _l3_missing(["K1"], FAKE_LOG_COMMITS_ABOVE_THE_NEXT_MARKER, [],
                       round_no=105) == ["K1"]
    assert _l3_missing(["C8"], FAKE_LOG_COMMITS_ABOVE_THE_NEXT_MARKER, [],
                       round_no=105) == []


# Маркера круга 105 нет — его унёс сорванный `hand` (тот самый круг 107
# из D0, пункт 2). Ниже по курсу — маркер 104.
FAKE_LOG_MISSING_MARKER_FLOOR_IS_EARLIER = (
    "\x1e8888888 Эстафета: круг 106, ход у coordinator — agent/TASK-89.md\n"
    "\n"
    "agent/BATON.json\n"
    "\x1e7777777 ТЗ-88 C7: работа без своего маркера\n"
    "\n"
    "rusterm/orphan.py\n"
    "\x1e4444444 Эстафета: круг 104, ход у coordinator — agent/TASK-87.md\n"
    "\n"
    "agent/BATON.json\n"
    "\x1e5555555 ТЗ-87 B1: работа круга 104\n"
    "\n"
    "rusterm/older.py\n"
)


def test_the_nearest_earlier_marker_is_the_floor_when_ours_is_gone():
    """ЗУБ D3 (второй): маркера N нет, есть N−1 — работа между N−1 и
    N+1 засчитана кругу N.

    Замок, а не краснота: прежний код (6cf14ce) на этом же литерале
    давал то же `[]`, потому что `marker_seen = True` там ставился от
    ЛЮБОГО маркера. Правило `number <= round_no` меняет форму с
    маркером ВЫШЕ N+1 — там прежний код кредитовал чужой круг, и на
    эту форму зуб уже есть (test_leading_marker_of_another_round_is_not
    _a_lower_bound, не тронут). Так что красный до правки у этого зуба
    не было, и отчёт говорит об этом прямо вместо того, чтобы
    приписывать себе чужую красноту."""
    assert _l3_missing(["C7"], FAKE_LOG_MISSING_MARKER_FLOOR_IS_EARLIER, [],
                       round_no=105) == []
