...
def test_recorded_model_response_replays_whole_pipeline_offline(app):
    ...
    ни одного факта; путь покрыт на машине без ключа. Ответ записан
    ДО ТЗ-35 G5: его цитаты — буквальные копии прежнего сплющенного
    текста и после починки извлечения в новый (табличный) текст
    строковым законом не находятся — unverified честно стоит."""
    conn, paths, _doc = app
