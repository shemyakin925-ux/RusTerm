...
NULL_REASONS: frozenset[str] = frozenset({
    ...
    # ТЗ-55 Y1: факт был, но его период вычищен окном давности снапшота
    # — «подавали давно», а не «никогда не подавали»; продолжение несёт
    # концепт и последний известный период (…: capex: last 2018-12-31)
    "stale_data",
    # ТЗ-82 E2: вход или готовое частное — NaN либо ±inf. Это не
    # missing_data: факт на месте, испорчен он, и подмена «данных нет»
    # спрятала бы повреждённый источник (замер координатора на dd11fbd:
