...
Бесплатный лимит OpenRouter считается на аккаунт, а не на модель:
цепочка запасных моделей спасает от отказа конкретной модели, но не
от исчерпания дневной квоты. Работай так, будто запросов немного:
один вызов — один законченный шаг, файлы читаются целиком и однократно.

Модель, на которой сделан каждый инкремент, фиксируется в `STATE.json`
и в отчёте. После смены модели — полный прогон тестов до написания
нового кода.
