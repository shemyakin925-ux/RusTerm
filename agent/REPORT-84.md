...
| 56 | первая попытка коммита K8, затем повтор после правки сообщения (`git commit -q -F …`, лог `/tmp/rt84-commit-k8.log`) | первая попытка отклонена: `SELFCHECK FAIL (L1): … сообщение называет agent/p1_rule.sh, но страж не в составе коммита` — в тексте сообщения стоял токен `P1`; лог первой попытки перезаписан повтором, поэтому цитата сохранилась только здесь. Вторая попытка (токенов стражей в сообщении нет, замер wall-времени в тексте исправлен с `8.2…8.5 с` на измеренные `8.4…9.8 с`): `P1: OK (HEAD~1..HEAD)`, `Итог: пройдено 13, провалено 0`, `Принято.`, `SELFCHECK OK`, `commit exit=0` → `8e111ca`, запушено (`335e5e2..8e111ca`). Эта строка и штамп `agent/STATE.json` (`last_commit: 8e111ca`) легли на ветку коммитом эстафеты — отдельного коммита ради двух записей не было |
