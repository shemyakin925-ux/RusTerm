...
- **The file is untrusted input.** It is the first thing in this project
  that a user brings in from outside, and `.docx`/`.xlsx` are zip
  archives full of XML — the classic shape of this bug class. Therefore:
  a hard ceiling on decompressed bytes and on member count (a zip bomb
  is refused as a value, not survived); **no member is ever written to
  disk by path from the archive** — parse in memory, and never join an
  archive-supplied name onto a filesystem path; XML parsed with external
  entities and DTD loading **off** (`defusedxml` if present, otherwise
  the stdlib parser explicitly configured — and say which in the
  docstring); a wall-clock ceiling per document so a crafted file cannot
  hang the night. Each of these is a **value**, not an exception:
  `format_unsupported` or a new local refusal, reported and moved past.
...
`format_unsupported`) and again with them present; a test builds a zip
bomb, a member whose name is `../../escape`, and an XML with an external
entity, and asserts all three are refused **by value** with nothing
written outside the data root; a test builds a
